...
bool XMLParser::initWithFile(const char *xmlFileName)
{
	...
	std::string fPath=CCFileUtils::sharedFileUtils()->fullPathForFilename(xmlFileName);
	char *str=new char[fPath.size()+1];
	memcpy(str,fPath.c_str(),fPath.size()+1);
	const char *fullPath = str;
	bool ret=_parser.parse(fullPath);
	delete str;
	return ret;
}
